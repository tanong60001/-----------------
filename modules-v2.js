...
async function viewDebtHistory(customerId, name) {
  const {data:payments} = await db.from('ชำระหนี้').select('*').eq('customer_id',customerId).order('date',{ascending:false}).limit(50);
  const {data:bills} = await db.from('บิลขาย').select('*').eq('customer_id',customerId).eq('status','ค้างชำระ').order('date',{ascending:false}).limit(50);
  ...
  (bills || []).forEach(b => {
    const debtAmt = (b.total || 0) - (b.deposit_amount || 0);
    if (debtAmt > 0) {
      history.push({
        date: b.date,
        type: 'debt',
  ...
  openModal(`ประวัติลูกหนี้: ${name}`, `
    <div style="max-height:400px;overflow-y:auto;">
      ${history.length === 0 ? '<p style="text-align:center;color:var(--text-tertiary);padding:30px;">ยังไม่มีประวัติ</p>' :
        `<table class="data-table">
          <thead><tr><th>วันที่</th><th>รายการ</th><th class="text-right">จำนวนเงิน</th><th>วิธี/สถานะ</th><th>พนักงาน</th></tr></thead>
          <tbody>${history.map(item => `<tr>
            <td>${formatDateTime(item.date)}</td>
            <td>${item.desc}</td>
            <td class="text-right"><strong style="color:${item.type === 'payment' ? 'var(--success)' : 'var(--danger)'}">${item.type === 'payment' ? '+' : '-'}฿${formatNum(item.amount)}</strong></td>
            <td>${item.method}</td>
            <td>${item.staff}</td>
          </tr>`).join('')}</tbody>
        </table>`}
