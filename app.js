...
async function updateAlerts() {
  ...
  try {
    ...
    if (!session || session.length === 0) alerts.push({ type: 'info', icon: 'account_balance_wallet', text: 'ยังไม่ได้เปิดลิ้นชักวันนี้' });
  } catch { }
...
async function renderCustomers() {
  ...
  section.innerHTML = `
    <div style="max-width:1200px; margin:0 auto; padding-bottom:30px; animation: fade-in-up 0.4s ease-out;">
      <div style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border-radius: 16px; padding: 24px; margin-bottom: 24px; border: 1px solid #bfdbfe; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(59, 130, 246, 0.05); flex-wrap: wrap; gap: 16px;">
        <div style="display: flex; align-items: center; gap: 16px;">
          <div style="width: 56px; height: 56px; background: #fff; border-radius: 14px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.1);">
            <i class="material-icons-round" style="font-size: 32px; color: #2563eb;">groups</i>
          </div>
          <div>
            <h2 style="margin: 0; font-size: 24px; color: #1e40af; font-weight: 700; font-family: var(--font-display);">ลูกค้าประจำ</h2>
            <div style="color: #3b82f6; font-size: 14px; margin-top: 4px;">ฐานข้อมูลลูกค้าและประวัติการซื้อ</div>
          </div>
        </div>
        <div style="display:flex; gap:12px; align-items:center;">
          <button class="btn" onclick="showAddCustomerModal()" style="background: linear-gradient(to right, #3b82f6, #2563eb); color: white; border: none; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.25); border-radius: 8px; padding: 8px 16px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; cursor: pointer;">
            <i class="material-icons-round" style="font-size: 18px;">person_add</i> เพิ่มลูกค้า
          </button>
        </div>
      </div>
      
      <div id="customer-stats" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px;"></div>
      ...
      <div style="background: #fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.03); border: 1px solid #e2e8f0; overflow: hidden;">
        <div style="padding: 16px 20px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; background: #f8fafc; flex-wrap: wrap; gap: 12px;">
          <h3 style="margin: 0; font-size: 16px; color: #334155; font-weight: 600;">รายชื่อลูกค้า</h3>
          <div style="position: relative; width: 100%; max-width: 300px;">
            ...
            <input type="text" id="customer-page-search" placeholder="ค้นหาชื่อ, เบอร์โทร..." oninput="loadCustomerData()" style="width: 100%; padding: 8px 12px 8px 40px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='#cbd5e1'">
          </div>
        </div>
        <div style="overflow-x: auto;">
          <table class="data-table" style="width: 100%; border-collapse: collapse; margin: 0; border: none; white-space: nowrap;">
            <thead>
              <tr style="background: #fff; border-bottom: 2px solid #f1f5f9;">
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: left;">ชื่อ-นามสกุล</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: left;">ติดต่อ</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: right;">ยอดซื้อสะสม</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: center;">ครั้งที่มา</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: right;">หนี้คงค้าง</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: right;">จัดการ</th>
              </tr>
            </thead>
...
async function loadCustomerData() {
  ...
  if (statsEl) {
    const activeCust = filtered.filter(c => (c.visit_count || 0) > 0).length;
    statsEl.innerHTML = `
      <div style="background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
        ...
        <div>
          <div style="color: #64748b; font-size: 13px; font-weight: 500; margin-bottom: 4px;">ลูกค้าทั้งหมด</div>
          <div style="font-size: 24px; font-weight: 700; color: #1e293b;">${filtered.length} คน</div>
        </div>
      </div>
      <div style="background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
        <div style="width: 48px; height: 48px; border-radius: 12px; background: #f0fdf4; display: flex; align-items: center; justify-content: center; color: #10b981;">
          <i class="material-icons-round">local_mall</i>
        </div>
        <div>
          <div style="color: #64748b; font-size: 13px; font-weight: 500; margin-bottom: 4px;">ลูกค้าที่เคยซื้อ</div>
          <div style="font-size: 24px; font-weight: 700; color: #1e293b;">${activeCust} คน</div>
        </div>
      </div>`;
  }
  ...
  tbody.innerHTML = filtered.length === 0 ? `
      <tr><td colspan="6" style="padding: 40px; text-align: center; color: #94a3b8;">
        <i class="material-icons-round" style="font-size: 48px; color: #cbd5e1; margin-bottom: 12px;">person_off</i>
        <div style="font-size: 16px; font-weight: 500; color: #64748b;">ไม่พบข้อมูลลูกค้า</div>
      </td></tr>` : filtered.map(c => {
      ...
      <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
        <td style="padding: 14px 20px;">
          <div style="display: flex; align-items: center; gap: 12px;">
            <div style="width: 40px; height: 40px; border-radius: 50%; background: #eff6ff; border: 1px solid #bfdbfe; display: flex; align-items: center; justify-content: center; color: #2563eb; font-weight: 700; font-size: 16px;">
              ${initial}
            </div>
            <div>
              <div style="font-weight: 600; color: #1e293b; font-size: 15px;">${c.name}</div>
              ${c.line_id ? `<div style="font-size: 12px; color: #059669; margin-top: 2px;"><i class="material-icons-round" style="font-size:12px; vertical-align:middle;">chat</i> ${c.line_id}</div>` : ''}
            </div>
          </div>
        </td>
        <td style="padding: 14px 20px; color: #475569;">
          <div style="font-weight: 500;">${c.phone || '-'}</div>
          ${c.email ? `<div style="font-size: 12px; color: #94a3b8; margin-top: 2px;">${c.email}</div>` : ''}
        </td>
        <td style="padding: 14px 20px; text-align: right;">
          <strong style="color: #3b82f6; font-size: 15px;">฿${formatNum(c.total_purchase)}</strong>
        </td>
        <td style="padding: 14px 20px; text-align: center; color: #475569; font-weight: 500;">
          <div style="display: inline-flex; align-items: center; justify-content: center; background: #f1f5f9; padding: 4px 10px; border-radius: 12px; font-size: 13px;">
            ${c.visit_count || 0} ครั้ง
          </div>
        </td>
        <td style="padding: 14px 20px; text-align: right;">
          ${(c.debt_amount || 0) > 0 ? `<strong style="color: #ef4444; font-size: 15px; background: #fee2e2; padding: 4px 8px; border-radius: 6px;">฿${formatNum(c.debt_amount)}</strong>` : '<span style="color: #94a3b8;">ไม่มีหนี้</span>'}
        </td>
        <td style="padding: 14px 20px; text-align: right;">
          <div style="display: inline-flex; gap: 4px;">
            ${(c.debt_amount || 0) > 0 ? `
              <button onclick="recordDebtPayment('${c.id}','${sn}')" style="background: transparent; border: 1px solid #d1fae5; color: #059669; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s;" onmouseover="this.style.background='#d1fae5'; this.style.color='#047857'" onmouseout="this.style.background='transparent'; this.style.color='#059669'" title="รับชำระหนี้">
                <i class="material-icons-round" style="font-size: 16px;">payments</i>
              </button>
            ` : ''}
            <button onclick="viewCustomerHistory('${c.id}','${sn}')" style="background: transparent; border: 1px solid #e0e7ff; color: #4f46e5; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s;" onmouseover="this.style.background='#e0e7ff'; this.style.color='#4338ca'" onmouseout="this.style.background='transparent'; this.style.color='#4f46e5'" title="ประวัติการซื้อ">
              <i class="material-icons-round" style="font-size: 16px;">history</i>
            </button>
            <button onclick="editCustomer('${c.id}')" style="background: transparent; border: 1px solid #f1f5f9; color: #64748b; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s;" onmouseover="this.style.background='#f1f5f9'; this.style.color='#475569'" onmouseout="this.style.background='transparent'; this.style.color='#64748b'" title="แก้ไข">
...
async function _oldRecordDebtPayment(customerId, name) {
  ...
  section.innerHTML = `
    <div style="max-width:1200px; margin:0 auto; padding-bottom:30px; animation: fade-in-up 0.4s ease-out;">
      <div style="background: linear-gradient(135deg, #fdf2f8 0%, #fce7f3 100%); border-radius: 16px; padding: 24px; margin-bottom: 24px; border: 1px solid #fbcfe8; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(236, 72, 153, 0.05); flex-wrap: wrap; gap: 16px;">
        <div style="display: flex; align-items: center; gap: 16px;">
          <div style="width: 56px; height: 56px; background: #fff; border-radius: 14px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(236, 72, 153, 0.1);">
            <i class="material-icons-round" style="font-size: 32px; color: #db2777;">account_balance_wallet</i>
          </div>
          <div>
            <h2 style="margin: 0; font-size: 24px; color: #be185d; font-weight: 700; font-family: var(--font-display);">รายจ่ายร้าน</h2>
            <div style="color: #db2777; font-size: 14px; margin-top: 4px;">บันทึกและจัดการค่าใช้จ่ายต่างๆ</div>
          </div>
        </div>
        <div style="display:flex; gap:12px; align-items:center; flex-wrap: wrap;">
          <input type="date" class="form-input" id="exp-date" value="${today}" style="width:160px; border-color:#f9a8d4; background:#fff;" onchange="loadExpenseData()">
          <button class="btn" onclick="showAddExpenseModal()" style="background: linear-gradient(to right, #db2777, #be185d); color: white; border: none; box-shadow: 0 4px 12px rgba(219, 39, 119, 0.25); border-radius: 8px; padding: 8px 16px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px; cursor: pointer;">
            <i class="material-icons-round" style="font-size: 18px;">add</i> บันทึกรายจ่าย
          </button>
        </div>
      </div>
      
      <!-- KPIs -->
      <div id="exp-stats" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px;"></div>

      <div style="background: #fff; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.03); border: 1px solid #e2e8f0; overflow: hidden;">
        <div style="padding: 16px 20px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; background: #f8fafc; flex-wrap: wrap; gap: 12px;">
          <h3 style="margin: 0; font-size: 16px; color: #334155; font-weight: 600;">รายการใช้จ่าย</h3>
          <div style="position: relative; width: 100%; max-width: 300px;">
            <i class="material-icons-round" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #94a3b8; font-size: 20px;">search</i>
            <input type="text" id="exp-search" placeholder="ค้นหารายการ / หมวดหมู่..." oninput="loadExpenseData()" style="width: 100%; padding: 8px 12px 8px 40px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 14px; outline: none; transition: border-color 0.2s;" onfocus="this.style.borderColor='#db2777'" onblur="this.style.borderColor='#cbd5e1'">
          </div>
        </div>
        <div style="overflow-x: auto;">
          <table class="data-table" style="width: 100%; border-collapse: collapse; margin: 0; border: none; white-space: nowrap;">
            <thead>
              <tr style="background: #fff; border-bottom: 2px solid #f1f5f9;">
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase;">วันเวลา</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase;">รายการ</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase;">หมวดหมู่</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase;">ชำระผ่าน</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: right;">จำนวนเงิน</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase;">บันทึกโดย</th>
                <th style="padding: 14px 20px; color: #64748b; font-weight: 600; font-size: 13px; text-transform: uppercase; text-align: right;">จัดการ</th>
              </tr>
            </thead>
  ...
  if (statsEl) {
    statsEl.innerHTML = `
      <div style="background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
        ...
        <div>
          <div style="color: #64748b; font-size: 13px; font-weight: 500; margin-bottom: 4px;">รายจ่ายรวมวันนี้</div>
          <div style="font-size: 24px; font-weight: 700; color: #ef4444;">฿${formatNum(total)}</div>
        </div>
      </div>
      <div style="background: #fff; border-radius: 12px; padding: 20px; border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
        <div style="width: 48px; height: 48px; border-radius: 12px; background: #f1f5f9; display: flex; align-items: center; justify-content: center; color: #64748b;">
          <i class="material-icons-round">receipt_long</i>
        </div>
        <div>
          <div style="color: #64748b; font-size: 13px; font-weight: 500; margin-bottom: 4px;">จำนวนรายการ</div>
          <div style="font-size: 24px; font-weight: 700; color: #1e293b;">${filtered.length} รายการ</div>
        </div>
      </div>`;
  }
  ...
  if (tbody) {
    tbody.innerHTML = filtered.length === 0 ? `
      <tr><td colspan="7" style="padding: 40px; text-align: center; color: #94a3b8;">
        <i class="material-icons-round" style="font-size: 48px; color: #cbd5e1; margin-bottom: 12px;">receipt</i>
        <div style="font-size: 16px; font-weight: 500; color: #64748b;">ไม่มีรายการรายจ่ายในวันนี้</div>
      </td></tr>` : filtered.map(e => {
        const dStr = formatDateTime(e.date) || '';
        const datePart = dStr.split(' ')[0] || '';
        const timePart = dStr.split(' ')[1] || '';
        
        return `
      <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'">
        <td style="padding: 14px 20px; color: #475569;">
          <div style="font-weight: 500; color: #1e293b;">${datePart}</div>
          <div style="font-size: 12px; color: #94a3b8;">${timePart}</div>
        </td>
        <td style="padding: 14px 20px; white-space: normal; min-width: 200px;">
          <div style="font-weight: 600; color: #1e293b;">${e.description}</div>
          ${e.note ? `<div style="font-size: 12px; color: #64748b; margin-top: 2px; display: flex; align-items: center; gap: 4px;"><i class="material-icons-round" style="font-size:12px;">notes</i> ${e.note}</div>` : ''}
        </td>
        <td style="padding: 14px 20px;">
          <span style="display: inline-flex; align-items: center; padding: 4px 10px; border-radius: 20px; background: #fef3c7; color: #b45309; font-size: 12px; font-weight: 600;">
            ${e.category || 'ทั่วไป'}
          </span>
        </td>
        <td style="padding: 14px 20px; color: #475569; font-weight: 500;">
          <div style="display: flex; align-items: center; gap: 6px;">
            <i class="material-icons-round" style="font-size:16px; color:${e.method === 'เงินสด' ? '#059669' : '#2563eb'}">${e.method === 'เงินสด' ? 'payments' : 'account_balance'}</i>
            ${e.method}
          </div>
        </td>
        <td style="padding: 14px 20px; text-align: right;">
          <strong style="color: #ef4444; font-size: 15px;">฿${formatNum(e.amount)}</strong>
        </td>
        <td style="padding: 14px 20px; color: #64748b;">
          <div style="display:flex; align-items:center; gap:8px;">
            <div style="width:28px; height:28px; border-radius:50%; background:#e2e8f0; display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:700; color:#475569;">
        ...
        <td style="padding: 14px 20px; text-align: right;">
          <button onclick="deleteExpense('${e.id}')" style="background: transparent; border: 1px solid #fee2e2; color: #ef4444; width: 32px; height: 32px; border-radius: 8px; cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s;" onmouseover="this.style.background='#fee2e2'; this.style.color='#dc2626'" onmouseout="this.style.background='transparent'; this.style.color='#ef4444'" title="ลบรายการ">
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.nav-item[data-page]').forEach(item => item.addEventListener('click', () => {
    ...
    // Auto-close sidebar drawer after navigating
    document.getElementById('sidebar')?.classList.remove('show');
