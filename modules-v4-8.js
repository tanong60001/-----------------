...
window.closeCashSessionWithCount = async function (session, expectedBalance) {
  ...
  window._v4States['cls_session'] = session;
  window._v4States['cls_expected'] = expectedBalance;
  window._v4States['cls_note'] = '';
  ...
  ov.style.cssText = 'position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.72);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;padding:16px;';
  ov.innerHTML = `<div id="v4-close-inner" style="background:var(--bg-surface);border-radius:20px;width:100%;max-width:580px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 64px rgba(0,0,0,.5);"></div>`;
  document.body.appendChild(ov);
  v4RenderCountStep();
};

function v4RenderCountStep() {
  const inner = document.getElementById('v4-close-inner');
  if (!inner) return;
  const state = window._v4States['cls'];
  const expectedBalance = window._v4States['cls_expected'];
  inner.innerHTML = `
    <div style="background:linear-gradient(135deg,#7c3aed,#a855f7);padding:18px 22px;flex-shrink:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
          <div style="font-size:10px;color:rgba(255,255,255,.65);text-transform:uppercase;letter-spacing:1px;margin-bottom:3px;">ปิดรอบเงินสด &nbsp;·&nbsp; ขั้นที่ 1 จาก 2</div>
          <div style="font-size:17px;font-weight:700;color:#fff;">นับยอดจริงในลิ้นชัก</div>
        </div>
        <div style="display:flex;gap:6px;">
          <div style="width:28px;height:6px;border-radius:3px;background:#fff;opacity:1;"></div>
          <div style="width:28px;height:6px;border-radius:3px;background:#fff;opacity:.3;"></div>
        </div>
      </div>
    </div>
    ...
    <div style="padding:14px 18px;border-top:1px solid var(--border-light);background:var(--bg-base);flex-shrink:0;">
      ...
      <div style="display:flex;gap:10px;">
        <button onclick="document.getElementById('v4-close-overlay').remove()"
          style="flex:1;padding:12px;border:1.5px solid var(--border-default);border-radius:var(--radius-md);background:none;cursor:pointer;font-family:var(--font-thai);font-size:14px;color:var(--text-secondary);">ยกเลิก</button>
        <button id="v4-close-confirm-btn" onclick="v4ShowConfirmStep()"
          style="flex:2;padding:12px;border:none;border-radius:var(--radius-md);background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;font-family:var(--font-thai);font-size:14px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 14px rgba(124,58,237,.3);">
          <i class="material-icons-round" style="font-size:18px;">navigate_next</i> ดูสรุปก่อนบันทึก
        </button>
      </div>
    </div>`;
  // Restore note if user came back from step 2
  const savedNote = window._v4States['cls_note'];
  if (savedNote) { const n = document.getElementById('v4-close-note'); if (n) n.value = savedNote; }
  // Sync counters
  window._v4States['cls_onChange']?.();
}

window.v4ShowConfirmStep = function() {
  const state = window._v4States['cls'];
  const expectedBalance = window._v4States['cls_expected'];
  const counted = v4Total(state);
  const diff = counted - expectedBalance;
  const note = document.getElementById('v4-close-note')?.value || '';
  window._v4States['cls_note'] = note;

  const diffColor = diff === 0 ? '#10B981' : diff > 0 ? '#F59E0B' : '#EF4444';
  const diffBg    = diff === 0 ? '#ECFDF5' : diff > 0 ? '#FFFBEB' : '#FEF2F2';
  const diffBdr   = diff === 0 ? '#6EE7B7' : diff > 0 ? '#FCD34D' : '#FCA5A5';
  const diffIcon  = diff === 0 ? 'check_circle' : diff > 0 ? 'trending_up' : 'trending_down';
  const diffLabel = diff === 0 ? 'ยอดตรงพอดี!' : diff > 0 ? `เงินเกิน +฿${formatNum(diff)}` : `เงินขาด −฿${formatNum(Math.abs(diff))}`;
  const diffSub   = diff === 0 ? 'ยอดที่นับได้ตรงกับที่ระบบบันทึกไว้' : 'มีผลต่างระหว่างยอดระบบกับยอดที่นับได้';
  ...
  inner.innerHTML = `
    <div style="background:linear-gradient(135deg,#7c3aed,#a855f7);padding:18px 22px;flex-shrink:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div>
          <div style="font-size:10px;color:rgba(255,255,255,.65);text-transform:uppercase;letter-spacing:1px;margin-bottom:3px;">ปิดรอบเงินสด &nbsp;·&nbsp; ขั้นที่ 2 จาก 2</div>
          <div style="font-size:17px;font-weight:700;color:#fff;">ยืนยันการปิดรอบ</div>
        </div>
        <div style="display:flex;gap:6px;">
          <div style="width:28px;height:6px;border-radius:3px;background:#fff;opacity:.3;"></div>
          <div style="width:28px;height:6px;border-radius:3px;background:#fff;opacity:1;"></div>
    ...
    <div style="padding:20px 22px;flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:14px;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        <div style="background:var(--bg-base);border-radius:14px;padding:16px 14px;text-align:center;border:1px solid var(--border-light);">
          <div style="font-size:10px;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">ระบบคาดว่ามี</div>
          <div style="font-size:22px;font-weight:800;color:var(--primary);font-family:var(--font-display);">฿${formatNum(expectedBalance)}</div>
        </div>
        <div style="background:var(--bg-base);border-radius:14px;padding:16px 14px;text-align:center;border:1px solid var(--border-light);">
          <div style="font-size:10px;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px;">นับได้จริง</div>
          <div style="font-size:22px;font-weight:800;color:${counted >= expectedBalance ? '#10B981' : '#EF4444'};font-family:var(--font-display);">฿${formatNum(counted)}</div>
        </div>
      </div>
      <div style="background:${diffBg};border:2px solid ${diffBdr};border-radius:14px;padding:18px;display:flex;align-items:center;gap:14px;">
        <i class="material-icons-round" style="color:${diffColor};font-size:38px;flex-shrink:0;">${diffIcon}</i>
        <div>
          <div style="font-size:16px;font-weight:800;color:${diffColor};margin-bottom:3px;">${diffLabel}</div>
          <div style="font-size:12px;color:var(--text-secondary);">${diffSub}</div>
        </div>
      </div>
      ${note ? `<div style="background:var(--bg-base);border-radius:12px;padding:12px 14px;border:1px solid var(--border-light);display:flex;align-items:center;gap:9px;"><i class="material-icons-round" style="font-size:17px;color:var(--text-tertiary);flex-shrink:0;">notes</i><span style="font-size:13px;color:var(--text-secondary);">หมายเหตุ: <strong style="color:var(--text-primary);">${note}</strong></span></div>` : ''}
    </div>
    <div style="padding:14px 18px;border-top:1px solid var(--border-light);background:var(--bg-base);flex-shrink:0;display:flex;gap:10px;">
      <button onclick="v4BackToCount()"
        style="flex:1;padding:12px;border:1.5px solid var(--border-default);border-radius:var(--radius-md);background:none;cursor:pointer;font-family:var(--font-thai);font-size:14px;color:var(--text-secondary);display:flex;align-items:center;justify-content:center;gap:6px;">
        <i class="material-icons-round" style="font-size:16px;">arrow_back</i> ย้อนกลับนับ
      </button>
      <button id="v4-close-save-btn" onclick="v4DoSaveClose()"
        style="flex:2;padding:12px;border:none;border-radius:var(--radius-md);background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;font-family:var(--font-thai);font-size:14px;font-weight:700;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 14px rgba(124,58,237,.35);">
        <i class="material-icons-round" style="font-size:18px;">lock</i> บันทึกปิดรอบ
      </button>
    </div>`;
};

window.v4BackToCount = function() { v4RenderCountStep(); };

window.v4DoSaveClose = async function() {
  const state = window._v4States['cls'];
  const session = window._v4States['cls_session'];
  const expectedBalance = window._v4States['cls_expected'];
  const note = window._v4States['cls_note'] || '';
  const counted = v4Total(state);
  const diff = counted - expectedBalance;
  const btn = document.getElementById('v4-close-save-btn');
  if (btn) { btn.disabled=true; btn.innerHTML='<i class="material-icons-round" style="font-size:18px;animation:spin 1s linear infinite;">hourglass_empty</i> กำลังบันทึก...'; }
  try {
    ...
    if (btn) { btn.disabled=false; btn.innerHTML='<i class="material-icons-round" style="font-size:18px;">lock</i> บันทึกปิดรอบ'; }
  }
};

// Backward-compat alias (kept for safety)
window.v4ConfirmClose = window.v4ShowConfirmStep;
