...
window.v12ReturnBill = async function(billId) {
  try {
    ...
    for (const it of (items||[])) {
      ...
      returnItems.push({name:it.name,qty:rq,price:it.price||0,unit:su,total:rq*(it.price||0),cost:it.cost||0,return_cost:(it.cost||0)*rq});
    }
    // บันทึก return_info
    const returnTotal = returnItems.reduce((s,ri)=>s+(ri.total||0),0);
    const returnCostTotal = returnItems.reduce((s,ri)=>s+(ri.return_cost||0),0);
    const origTotal = bill.return_info?.original_total || bill.total || 0;
    await db.from('บิลขาย').update({status:'คืนสินค้า',delivery_status:'ยกเลิก',total:0,
      return_info:{returned_at:new Date().toISOString(),returned_by:_v20staff(),return_reason:'คืนทั้งบิล',
        return_items:returnItems,return_total:returnTotal,return_cost_total:returnCostTotal,original_total:origTotal,new_total:0}
    }).eq('id',billId);
    // หักหนี้/ยอดซื้อ — คำนวณหนี้จริง (หักมัดจำ)
    if(bill.customer_id){
      try{
        const{data:c}=await db.from('customer').select('*').eq('id',bill.customer_id).single();
        if(c){
          const isDebt = ['ค้างชำระ','ค้างเครดิต'].includes(bill.method) || bill.status==='ค้างชำระ';
          const depositPaid = bill.deposit_amount||0;
          const actualDebt = isDebt ? Math.max(0, origTotal - depositPaid) : 0;
          const debtDeduction = Math.min(actualDebt, c.debt_amount||0);
          await db.from('customer').update({
            total_purchase:Math.max(0,(c.total_purchase||0)-origTotal),
            debt_amount:Math.max(0,(c.debt_amount||0)-debtDeduction)
          }).eq('id',bill.customer_id);
        }
      }catch(_){}
    }
    typeof toast==='function'&&toast('คืนสินค้าสำเร็จ — สต็อกคืนตาม conv_rate','success');
    if(typeof loadProducts==='function') await loadProducts(); if(typeof updateHomeStats==='function') updateHomeStats();
    if(typeof v12BMCLoad==='function') v12BMCLoad(); if(typeof renderDebts==='function') renderDebts();
...
window.v20BMCPayDebt = async function(billId) {
  try {
    ...
    v12BMCLoad(); if(typeof renderDebts==='function') renderDebts();
