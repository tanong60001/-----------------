      ...
      if(a==='ok'){
        ...
        (async function(){
          try{
            ...
            if(typeof window.v9AutoUpdateBillStatus==='function') await window.v9AutoUpdateBillStatus(customerId);
      ...
      if(method==='เงินสด'){
        ...
        (async function(){
          try{
            ...
            if(typeof window.v9AutoUpdateBillStatus==='function') await window.v9AutoUpdateBillStatus(id);
            if(typeof loadCustomerData==='function') loadCustomerData();
            if(typeof renderDebts==='function') renderDebts();
...
window.v20BMCPayDebt = async function(billId) {
  try{
    ...
    var effectiveTotal = bill.return_info?.new_total ?? bill.total;
    var rem=Math.max(0, effectiveTotal-(bill.deposit_amount||0));
    if(rem<=0){if(typeof toast==='function') toast('ไม่มียอดค้างชำระ','info'); return;}
    ...
    var hasReturn = bill.return_info?.return_total > 0;

    var saveDebt=async function(paidAmt,method,recvTotal,chgTotal,recvD,chgD){
      var nd=(bill.deposit_amount||0)+paidAmt, full=nd>=effectiveTotal;
    ...
    if(hasReturn) {
      dialogRows.push({label:'⚠ มีการคืนสินค้า', val:'คืน ฿'+f2(bill.return_info.return_total), color:'#7c3aed'});
      dialogRows.push({label:'ยอดหลังคืน', val:'฿'+f2(effectiveTotal), color:'#d97706'});
    }
    if(bill.deposit_amount>0) dialogRows.push({label:'ชำระแล้ว', val:'฿'+f2(bill.deposit_amount), color:'#16a34a'});
    dialogRows.push({label:'ยอดค้างชำระ', val:'฿'+f2(rem), color:'#ef4444'});
