  ...
  return `
    <div class="v12-delivery-form">
      ...
      <div style="margin-bottom: 12px; display: flex; gap: 10px;">
        <div style="flex:1; border: 1.5px solid ${v12State.deliveryPaymentMode === 'pay_now' ? '#10b981' : '#d1d5db'}; background: ${v12State.deliveryPaymentMode === 'pay_now' ? '#ecfdf5' : '#fff'}; padding: 10px; border-radius: 8px; cursor: pointer; text-align: center; transition: all 0.2s ease;" onclick="v12SetDeliveryPaymentMode('pay_now')">
          <i class="material-icons-round" style="color: ${v12State.deliveryPaymentMode === 'pay_now' ? '#10b981' : '#9ca3af'}; font-size: 20px;">check_circle</i>
          <div style="font-weight: 700; color: ${v12State.deliveryPaymentMode === 'pay_now' ? '#047857' : '#374151'}; font-size: 13px; margin-top: 4px;">ชำระเลย</div>
        </div>
        <div style="flex:1; border: 1.5px solid ${v12State.deliveryPaymentMode === 'cod' ? '#f59e0b' : '#d1d5db'}; background: ${v12State.deliveryPaymentMode === 'cod' ? '#fffbeb' : '#fff'}; padding: 10px; border-radius: 8px; cursor: pointer; text-align: center; transition: all 0.2s ease;" onclick="v12SetDeliveryPaymentMode('cod')">
          <i class="material-icons-round" style="color: ${v12State.deliveryPaymentMode === 'cod' ? '#f59e0b' : '#9ca3af'}; font-size: 20px;">local_atm</i>
          <div style="font-weight: 700; color: ${v12State.deliveryPaymentMode === 'cod' ? '#b45309' : '#374151'}; font-size: 13px; margin-top: 4px;">ชำระหน้างาน</div>
        </div>
      </div>
      <div class="v12-form-row">
        <div class="v12-form-group">
...
window.v12SetDeliveryPaymentMode = function (mode) {
  v12State.deliveryPaymentMode = mode;
  const body = document.getElementById('v12-step-body');
  if (body) v12S2(body);
};
...
window.v12PrintDeliveryNote = async function (billId) {
  try {
    const { data: bill } = await db.from('บิลขาย').select('*').eq('id', billId).single();
    const { data: items } = await db.from('รายการในบิล').select('*').eq('bill_id', billId);
    const activeItems = (items || []).map(i => {
      let displayName = i.name;
      const tk = i.take_qty || 0;
      const dq = i.deliver_qty || 0;
      if (tk > 0) {
        if (dq > 0) displayName += ` (รับแล้ว ${tk}, ส่งรอบนี้ ${dq})`;
        else displayName += ` (ลูกค้ารับไปเองแล้ว ${tk})`;
      }
      return { ...i, name: displayName, qty: i.qty };
    });
...
async function v12DQRenderCards(bills) {
  ...
  area.innerHTML = bills.map(b => {
    ...
    let paymentBadge = '';
    if (b.status === 'ชำระหน้างาน') {
      paymentBadge = `<span style="background:#fffbeb;color:#d97706;padding:2px 6px;border-radius:4px;font-size:10px;font-weight:700;border:1px solid #fcd34d;"><i class="material-icons-round" style="font-size:10px;vertical-align:middle;margin-right:2px;">local_atm</i>เก็บเงินปลายทาง</span>`;
    } else if (b.status === 'สำเร็จ' || b.status === 'ชำระแล้ว') {
      paymentBadge = `<span style="background:#ecfdf5;color:#047857;padding:2px 6px;border-radius:4px;font-size:10px;font-weight:700;border:1px solid #6ee7b7;"><i class="material-icons-round" style="font-size:10px;vertical-align:middle;margin-right:2px;">check_circle</i>ชำระแล้ว</span>`;
    }
    ...
    <div class="v12-dq-card" id="dq-card-${b.id}">
      ...
      <div class="v12-dq-card-body">
        ...
        <div class="v12-dq-items">
          ${items.length
        ? items.map(it => `<div class="v12-dq-item"><span>${it.name}${(it.take_qty||0)>0?' <span style="color:#8b5cf6;font-size:11px">(รับแล้ว '+it.take_qty+' '+(it.unit||'ชิ้น')+')</span>':''}</span><strong>${it.deliver_qty} ${it.unit||'ชิ้น'}</strong></div>`).join('')
...
window.v12DQMarkDone = async function (billId) {
  ...
  try {
    ...
    const { data: bill } = await db.from('บิลขาย').select('status, deposit_amount, total, method, return_info').eq('id', billId).single();
    let newStatus = 'สำเร็จ';
    const effectiveTotal = bill ? (bill.return_info?.new_total ?? bill.total) : 0;
    const hasRemaining = bill && (bill.deposit_amount || 0) < effectiveTotal;
    ...
    if (hasRemaining && typeof v20BMCPayDebt === 'function') {
        setTimeout(() => {
            v20BMCPayDebt(billId);
        }, 600);
    }
...
window.v12BMCLoad = async function () {
  ...
  try {
    ...
    tbody.innerHTML = filtered.map(b => {
      ...
      const hasDebt = (b.status === 'ค้างชำระ') || (b.deposit_amount > 0 && b.deposit_amount < b.total && b.status !== 'ยกเลิก' && b.status !== 'คืนสินค้า');
      ...
      return `<tr>
        ...
        <td>
          <div style="display:flex;gap:4px;flex-wrap:wrap;">
            ...
            ${hasDebt ? `
            <button class="v12-bmc-action-btn" onclick="v20BMCPayDebt('${b.id}')" title="รับชำระ" style="color:#10b981; border-color:rgba(16,185,129,.3);">
              <i class="material-icons-round" style="font-size:13px">payments</i> รับชำระ
            </button>` : ''}
            ${b.status === 'สำเร็จ' || b.status === 'รอจัดส่ง' || b.status === 'จ่ายแล้วบางส่วน' ? `
            <button class="v12-bmc-action-btn danger" onclick="typeof v10ShowReturnModal==='function'?v10ShowReturnModal('${b.id}'):v12ReturnBill('${b.id}')" title="คืนสินค้า" style="color:#ef4444; border-color:rgba(239,68,68,0.2);">
