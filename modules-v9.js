      ...
      <tr style="${isOverdue ? 'background:#fff5f5;' : ''}">
        <td>
          <div class="payable-cust-name">
            ...
            <div>
              <div style="font-size:15px;line-height:1.2;">${supp.name || r.supplier_id?.slice(0, 8) + '…' || 'ไม่ระบุ'}</div>
              ${supp.phone ? `<div style="font-size:12px;color:#64748b;font-weight:400;margin-top:4px;"><i class="material-icons-round" style="font-size:12px;vertical-align:middle">phone</i> ${supp.phone}</div>` : ''}
            </div>
          </div>
        </td>
        ...
        <td style="text-align:center;">
          ${r.status === 'ค้างชำระ' ? `
            <div class="payable-actions">
              <button class="btn-pay-ap" onclick="window.v9PayCreditor('${r.id}','${(supp.name || '').replace(/'/g, "\\'")}',${balance},${amount})">
  ...
  try {
    ...
    for (const bill of bills) {
      ...
      if (bill.status !== newStatus || currentPaid !== paidForThisBill) {
        ...
        await db.from('บิลขาย').update({ status: newStatus, return_info: retInfo }).eq('id', bill.id);
      }
    }
  } catch (err) {
